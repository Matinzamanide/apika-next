...
export async function POST(req: Request) {
  try {
    ...
    const filterProducts = (query: string, filters: FilterOptions): Product[] => {
      return products.filter(p => {        const title = p.title?.toLowerCase() || '';
        ...
        if (filters.power !== undefined) {
          ...
          if (specValue) {
            ...
            const horseInSpec = powerInSpec >= 0.7 ? powerInSpec / 0.735 : powerInSpec * 1.36; 
    ...
    else {      const capacityMatch = userMessage.match(/(\d+)\s*لیتر/i);
